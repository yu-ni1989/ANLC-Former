...
def val_nodata_check(img, labels):
    return np.any(np.sum(img == 0, axis=2) == 4)

class StreamingGeospatialDataset(IterableDataset):
    
    def __init__(self, imagery_fns, label_fns=None, groups=None, chip_size=256, num_chips_per_tile=200, windowed_sampling=False, image_transform=image_transforms, label_transform=label_transforms, nodata_check=nodata_check, verbose=False):
        ...
        if label_fns is None:
            ...
            self.fns = list(zip(imagery_fns, label_fns))  # 遥感影像和标签组合
